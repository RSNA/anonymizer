...
**Version 19.0.10** ist die aktuelle stabile V19-Version auf dem Branch `master` und auf PyPI. Die Entwicklungs-Vorabversion **19.1.0.dev1** liegt auf dem Branch `dev` (`uv pip install --pre rsna-anonymizer`). Benötigt **Python 3.12–3.14** mit eingebautem **tkinter**. Bevorzugt **3.12** für die breiteste Tk/AI-Wheel-Abdeckung; **3.14.1** vermeiden (torchvision schließt diesen Patch aus).

## Installation

Sie brauchen drei Dinge: **uv**, eine Python-Umgebung **3.12–3.14** mit **tkinter** und (unter macOS, für AI Features) **libomp**.
...
Die Desktop-Oberfläche braucht **tkinter**. Installieren Sie ein Python mit tkinter, erstellen Sie die venv und installieren Sie **19.0.10**:

| Plattform | tkinter verfügbar machen |
| --- | --- |
| **Windows** | Python 3.12–3.14 von [python.org](https://www.python.org/downloads/) mit **Add to PATH** und **tcl/tk and IDLE** |
| **macOS** | Bevorzugt `uv python install 3.12` (Tk 9.x). Oder Homebrew: `brew install python@3.12 python-tk@3.12` |
| **Linux** | `sudo apt install python3.12 python3.12-tk python3.12-venv` (oder passende Pakete für 3.13 / 3.14) |

```bash
uv python install 3.12                     # oder: 3.13 / 3.14
uv venv rsna-anonymizer --python 3.12      # oder: --python 3.13 / 3.14
...
Für die 19.1-Entwicklung (Vorabversion) verwenden Sie `git checkout dev` statt `master`.
