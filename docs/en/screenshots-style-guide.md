...
```
docs/en/04-create-project/shots/
docs/en/05-search/shots/
docs/en/06-view/shots/
docs/en/07-process/01-ai-features-setup/shots/
docs/en/07-process/02-remove-burned-in-text/shots/   # davidson_cxr
docs/en/07-process/03-harmonize-names/shots/         # Brain_Ax_EarlyArt
docs/en/07-process/04-blur-faces/shots/              # Brain_Ax_EarlyArt Gaussian
docs/en/07-process/05-run-on-many-studies/shots/
docs/en/08-send/shots/
```
...
Requires a desktop display; Orthanc on `127.0.0.1:4242` (AE `ORTHANC`) for Search/Query shots; Screen Recording permission on macOS.
...
- Process demos use **non-synthetic** fixtures only:
  - **7.2** Remove burned-in text → `davidson_cxr` (whitelist → Frame → Detect → Remove)
  - **7.3** Harmonize → `Brain_Ax_EarlyArt` (Harmonize Description → Segment Brain Features)
  - **7.4** Face blur → `Brain_Ax_EarlyArt` (Gaussian)
  - **7.5** Batch → both fixtures selected in Dataset
- Soft-fail AI-heavy Process shots when models are missing
- Hard-fail Orthanc-dependent Search shots when C-ECHO fails

## Priority shots (V19)

Welcome; Create project settings (+ subdialogs); Dashboard; Search import/query; Dataset; Process (Davidson OCR, Brain_Ax Harmonize/Face, batch); Send.
