...
**Version 19.0.3** is the current V19 release on the `master` branch and PyPI. Requires **Python 3.11 or 3.12** with **tkinter** built in. Python 3.13 is not supported.
...
The desktop UI needs **tkinter**. Install a Python that ships with it, then create the venv and install **19.0.3**:
...
uv pip install rsna-anonymizer             # version 19.0.3
```
...
rsna-anonymizer --version # should report 19.0.3
```
