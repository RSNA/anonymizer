...
def create_view(view: ctk.CTkFrame):
    ...
    # Q/R SCP IP Address:
    scp_label = ctk.CTkLabel(view, text=_("DICOM Q/R SCP:"))
    ...
    scp_aet_entry = ctk.CTkEntry(
        view,
        width=int(ip_max_chars * char_width_px),
    ...
    scp_aet_entry.grid(row=0, column=1, pady=(PAD, 0), padx=PAD, sticky="nw")
    ...
    port_var = ctk.IntVar(view, value=scp_ip_port)
    ...
    entry_callback = lambda event: int_entry_change(
        event, port_var, ip_port_min, ip_port_max, __name__, "scp_ip_port"
    )
    ...
    scu_ip_var = ctk.StringVar(view, value=scp_ip_addr)
    ...
    aet_min_chars = 3
    aet_max_chars = 16
    scu_aet_var = ctk.StringVar(view, value=scp_aet)
