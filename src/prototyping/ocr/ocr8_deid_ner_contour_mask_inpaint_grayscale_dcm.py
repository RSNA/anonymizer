...
def process_grayscale_image(dcm_path: Path, nlp, reader):
    logger.info(f"Processing Grayscale Image: {dcm_path.name}")
    ...
    samples_per_pixel = source_ds.get("SamplesPerPixel", 1)
    ...
    for frame in range(no_of_frames):
        ...
        if "ModalityLUTSequence" in source_ds:
            pixels = apply_modality_lut(pixels, source_ds)
            logger.info(f"Applied Modality LUT: new pixels.value.range:[{pixels.min(), pixels.max()}]")
        elif has_voi_lut(source_ds):
...
def main():
    ...
    reader = Reader(["en", "de", "fr", "es"])  # for full range of Roman char sets include: de,fr,es
    ...
    for dcm_file in dcm_files:
        process_grayscale_image(dcm_file, nlp, reader)
