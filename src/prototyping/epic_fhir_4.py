    ...
    def exchange_auth_code(self, auth_code):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Failed to retrieve access token. Status: {response.status_code}"
                )
...
def fetch_patient_diagnostic_reports(access_token, patient_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Failed to retrieve diagnostic reports. Status: {response.status_code}"
            )
            pprint(response.json())
    except Exception as e:
...
def fetch_patient_diagnostic_reports_by_icd(access_token, patient_id, icd_code):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Failed to retrieve filtered diagnostic reports. Status: {response.status_code}"
            )
            pprint(response.json())
    except Exception as e:
...
def run_test(access_token):
    ...
    # fetch_patient_diagnostic_reports_by_icd(
    #     access_token, fhir_pt_id_1, fhir_pt_id_1_icd10
    # )
...
def start_server():
    ...
    ssl_context.load_cert_chain(
        certfile="src/certs/cert.pem", keyfile="src/certs/key.pem"
    )
