...
def start_local_storage_scp(temp_dir: str):
    assert storage_scp.start(
        TEST_LOCAL_SCP_IP,
        TEST_LOCAL_SCP_PORT,
        TEST_LOCAL_SCP_AET,
        local_storage_dir(temp_dir),
    )
    assert storage_scp.server_running()
...
def send_file_to_scp(pycidom_test_filename: str, to_pacs_simulator: bool):
    ...
    dcm_filename = get_testdata_file(pycidom_test_filename)
    if not dcm_filename:
        fetch_data_files()
    assert dcm_filename
    ux_Q = queue.Queue()
