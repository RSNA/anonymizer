...
def ensure_synthetic_ct_assets() -> dict[str, Path]:
    """Write committed phantom asset dirs when missing or stale (session-safe)."""
    head_dir = SYNTHETIC_CT_HEAD_ASSET_DIR
    version_file = head_dir / ".phantom_version"
    needs_generation = not head_dir.exists() or not list_dcm_files(head_dir)
    if not needs_generation:
        try:
            needs_generation = int(version_file.read_text(encoding="utf-8").strip()) < SYNTHETIC_PHANTOM_VERSION
        except (OSError, ValueError):
            needs_generation = True
    if needs_generation:
        import shutil

        for asset_dir in SYNTHETIC_CT_ASSET_DIRS.values():
            if asset_dir.exists():
                shutil.rmtree(asset_dir)
        assets = write_synthetic_phantom_assets()
        version_file = SYNTHETIC_CT_HEAD_ASSET_DIR / ".phantom_version"
