...
from src.anonymizer.model.anonymizer_sql import Base, AnonymizerModelSQL, PHI, Series, Study, UID, TagKeep
...
def test_load_script_empty(anonymizer_model, tmp_path):
    ...
    assert anonymizer_model._session.query(TagKeep).count() == initial_count, "Empty script should not insert any records"
...
def test_get_acc_no_count(anonymizer_model):
    ...
    anonymizer_model._session.add_all([
        Study(study_uid="STUDY-002", patient_id=patient_id, accession_number="ACC-1", anon_accession_number="ANON-1", anon_accession_number_count=1),
        Study(study_uid="STUDY-003", patient_id=patient_id, accession_number="ACC-2", anon_accession_number="ANON-2", anon_accession_number_count=2),
        Study(study_uid="STUDY-004", patient_id=patient_id, accession_number="ACC-3", anon_accession_number="ANON-3", anon_accession_number_count=5),
    ])
    anonymizer_model._session.commit()
...
def test_series_complete(anonymizer_model):
    ...
    assert not anonymizer_model.series_complete(ptid, study_uid, series_uid, target_count), "Series should not be complete"
    ...
    assert anonymizer_model.series_complete(ptid, study_uid, series_uid, target_count), "Series should be complete when instance count meets target"

    # Check with a higher target count (should return False)
    assert not anonymizer_model.series_complete(ptid, study_uid, series_uid, 15), "Series should not be complete when instance count is below target"
